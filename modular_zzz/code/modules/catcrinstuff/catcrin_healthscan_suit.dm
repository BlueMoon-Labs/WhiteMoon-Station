...
/obj/item/clothing/suit/armor/mark45_armor/Initialize(mapload)
	hudhealth_overlay = mutable_appearance(worn_icon, hudhealth_overlay_icon, -13)
	. = ..()
...
/obj/item/clothing/suit/armor/mark45_armor/proc/update_damage(mob/living/carbon/user)
	if(user.stat == DEAD)
		return
	var/healthpercent = (user.health/user.maxHealth) * 150
...
/obj/item/clothing/suit/armor/brigdoc/mark45_armor_medic/Initialize(mapload)
	hudhealth_overlay = mutable_appearance(worn_icon, hudhealth_overlay_icon, -13)
	. = ..()

/obj/item/clothing/suit/armor/brigdoc/mark45_armor_medic/equipped(mob/user, slot)
	..()
	if(slot_flags & slot)
		if(iscarbon(user) && user.stat != DEAD)
			user.add_overlay(hudhealth_overlay)
...
/obj/item/clothing/suit/armor/brigdoc/mark45_armor_medic/dropped(mob/user)
	..()
	user.cut_overlay(hudhealth_overlay)
	UnregisterSignal(user, COMSIG_CARBON_UDPATE_SUIT_AFTER_UPDATEHEALTH)

/obj/item/clothing/suit/armor/brigdoc/mark45_armor_medic/proc/update_damage(mob/living/carbon/user)
	if(user.stat == DEAD)
		return
	var/healthpercent = (user.health/user.maxHealth) * 150
	switch(healthpercent)
...
/obj/item/clothing/suit/armor/brigdoc/mark45_armor_medic/proc/update_healthbar_overlay(mob/living/carbon/user)
	user.cut_overlay(hudhealth_overlay)
	hudhealth_overlay = mutable_appearance(worn_icon, hudhealth_overlay_icon, -13)
	hudhealth_overlay_old_icon_state = hudhealth_overlay_icon
	user.add_overlay(hudhealth_overlay)
